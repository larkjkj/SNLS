...
extern u8* sn_Mwrite(emGeneral* emulator, u16 address, u16 offset, u32 value) {
	printf("bus_write: writing %X to [%X][%X]\n", value, emulator->memory->bank_target, emulator->memory->address_target);
	emulator->memory->bank_array[emulator->memory->bank_target][emulator->memory->address_target] = value;
	printf("%X\n", emulator->memory->bank_array[emulator->memory->bank_target][emulator->memory->address_target]);
...
extern u16 sn_Mread_u16_const(emGeneral* emulator, u16 offset) {
	...
	printf("bus_read: returning [%X] \n", holdAddr); 
	return holdAddr;
};

extern u16 sn_Mread_u24_const(emGeneral* emulator, u16 offset) {
	exit(1);
	holdLoAddr = *(++emulator->cpu->sn_PC);
	holdHiAddr = *(++emulator->cpu->sn_PC);
	holdAddr = (holdHiAddr << 8 | holdLoAddr);

	emulator->memory->bank_target = *(++emulator->cpu->sn_PC);
	getMappedBank(emulator->memory->bank_target, holdAddr, emulator);
}

extern u16 sn_Mread_u16_absolute(emGeneral* emulator, u16 offset) {
	holdLoAddr = *(++emulator->cpu->sn_PC);
	holdHiAddr = *(++emulator->cpu->sn_PC);
	holdAddr = (holdHiAddr << 8 | holdLoAddr); 
	...
	return emulator->memory->bank_array[emulator->cpu->sn_DBR][holdAddr];
};

extern u16 sn_Mread_u24_absolute(emGeneral* emulator, u16 offset) {
	holdLoAddr = *(++emulator->cpu->sn_PC);
	holdHiAddr = *(++emulator->cpu->sn_PC);
	holdAddr = (holdHiAddr << 8 | holdLoAddr);
